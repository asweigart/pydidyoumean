...
def printFileSuggestion(
    filename,
    message="Did you mean %s?\n",
    folder=".",
    threshold=2,
    includeIdenticalFilename=False,
):
  ...
  Args:
      ...
      includeIdenticalFilename (bool, optional) - If True, a filename identical
        ...
        False."""
    if skipBecauseOptimized():
        return

    sys.stdout.write(
        formatFileSuggestion(
            filename, message, folder, threshold, includeIdenticalFilename
        )
    )


def formatFileSuggestion(
    filename,
    message="Did you mean %s?\n",
    folder=".",
    threshold=2,
...
def suggestAllFiles(filename, folder=".", threshold=2, includeIdenticalFilename=False):
  ...
  Args:
      ...
      includeIdenticalFilename (bool, optional) - If True, a filename identical
        ...
        False."""
    filesAndDistances = [
        (f, levenshtein(filename, f))
        for f in os.listdir(folder)
        if includeIdenticalFilename or f != filename
    ]
    filesAndDistances = [
        x for x in filesAndDistances if x[1] <= threshold
    ]  # remove files over the threshold
    filesAndDistances.sort(
        key=lambda x: x[1]
    )  # sort by comparing the levenshtein distance in x[1]
...
def printSuggestion(
    name,
    possibleSuggestions=None,
    message="Did you mean %s?\n",
    threshold=2,
    includeIdenticalName=False,
):
  ...
  Args:
    ...
    sys.stdout.write(
        formatSuggestion(
            name, possibleSuggestions, message, threshold, includeIdenticalName
        )
    )


def formatSuggestion(
    name,
    possibleSuggestions=None,
    message="Did you mean %s?\n",
    threshold=2,
...
def suggestAll(name, possibleSuggestions=None, threshold=2, includeIdenticalName=False):
  ...
  Args:
    ...
    if possibleSuggestions is None:
        possibleSuggestions = []
    sugsAndDistances = [
        (n, levenshtein(name, n))
        for n in possibleSuggestions
        if includeIdenticalName or n != name
    ]
    sugsAndDistances = [
        x for x in sugsAndDistances if x[1] <= threshold
    ]  # remove names over the threshold
    sugsAndDistances.sort(
        key=lambda x: x[1]
    )  # sort by the levenshtein distance in x[1]
    ...
    for zz in range(0, s2_length):
        for sz in range(0, s1_length):
            if s1[sz] == s2[zz]:
                ...
                matrix[zz + 1][sz + 1] = min(
                    matrix[zz + 1][sz] + 1, matrix[zz][sz + 1] + 1, matrix[zz][sz] + 1
                )
